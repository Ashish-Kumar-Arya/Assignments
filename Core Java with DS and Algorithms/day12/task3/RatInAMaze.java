...
public class RatInAMaze {
	...
	public static void solveMaze(int r, int c, int n, int[][]maze, List<String> res, StringBuilder cp) {
		if(r==n-1&&c==n-1) {
			res.add(cp.toString());
			return;
		}
		...
		for(int i=0; i<n-1;i++) {
			...
			if(isValid(nextr,nextc,n,maze)) {
				...
				solveMaze(nextr, nextc, n, maze, res, cp); //calling the findPath()
	...
	public static void main(String[] args) {
		...
		if(maze[0][0]!=0&&maze[n-1][n-1]!=0)
			solveMaze(0,0,n,maze,res,cp);
		if(res.isEmpty())
			System.out.println("There is no way to reach the destination.");
		else {
			for(String p:res)
				System.out.print(p+" ");
			System.out.println();
